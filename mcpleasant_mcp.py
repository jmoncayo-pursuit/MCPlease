...
class MCPleasantMCPServer:
    ...
    def _explain_code(self, args: Dict[str, Any]) -> str:
        ...
        complexity_indicators = ["nested loops", "recursion", "multiple conditions"]
        if any(indicator in code.lower() for indicator in ["for " in code and "for " in code[code.find("for ")+4:], "def " in code and code.count("def ") > 1]):
    ...
    def _refactor_code(self, args: Dict[str, Any]) -> str:
        ...
        # Code quality
        if not any('"""' in line or "'''" in line for line in lines) and "def " in code:
